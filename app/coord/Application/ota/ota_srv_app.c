  ...
  else
  {
    ...
    if ( zclOTA_Permit && ( pParam != NULL ) )
    {
      ...
      else
      {
        ...
        status = cmd_OtaFileReadReq ( pSrcAddr, seqNo, &pParam->fileId, len, pParam->fileOffset );
...
ZStatus_t zclOTA_Srv_QueryNextImageReq ( afAddrType_t *pSrcAddr, uint8_t seqNo, zclOTA_QueryNextImageReqParams_t *pParam )
{
  ...
  if ( zclOTA_Permit )
  {
    ...
    status = cmd_OtaGetImage ( pSrcAddr, seqNo, &pParam->fileId, pParam->hardwareVersion, NULL, options );
  }
  else
  {
...
ZStatus_t zclOTA_Srv_UpgradeEndReq ( afAddrType_t *pSrcAddr, uint8_t seqNo, zclOTA_UpgradeEndReqParams_t *pParam )
{
  ...
  if ( zclOTA_Permit && ( pParam != NULL ) )
  {
    ...
    cmd_OtaSendStatus ( pSrcAddr, seqNo, MT_OTA_DL_COMPLETE, pParam->status, 0 );
 ...
 * @return  none
 */
void zclOTA_ServerHandleFileSysCb ( afAddrType_t dstAddr, uint8_t cmd, uint8_t seqNo, uint8_t param_len, uint8_t* paramData )
{
  zclOTA_FileID_t pFileId;
  uint8_t *pMsg;

  if ( (param_len > 0) && (paramData != NULL))
  {
    ...
    switch ( cmd )
    {
      case OTA_CMD_NEXT_IMG:
        zclOTA_ProcessNextImgRsp ( pMsg, &pFileId, &dstAddr, seqNo );
        break;

      case OTA_CMD_FILE_READ:
        zclOTA_ProcessFileReadRsp ( pMsg, &pFileId, &dstAddr, seqNo );
        break;
...
ZStatus_t zclOTA_Srv_QuerySpecificFileReq ( afAddrType_t *pSrcAddr, uint8_t seqNo, zclOTA_QuerySpecificFileReqParams_t *pParam )
{
  ...
  if ( zclOTA_Permit )
  {
    status = cmd_OtaGetImage ( pSrcAddr, seqNo, &pParam->fileId, 0,  pParam->nodeAddr, MT_OTA_QUERY_SPECIFIC_OPTION );
  }
  else
  {
    status = ZOtaNoImageAvailable;

 ...
 * @fn          ZDP_GetTransID
 *
 * @brief       Get current ZDP trans ID value
 *
 * @param       NONE
 *
* @return       trans ID value
 */
uint8_t ZDP_GetTransID( void )
{
  return ZDP_TransID;
}
                                   
/*********************************************************************
 * @fn          ZDP_ProcessDataConfirm
 *
 * @brief       process aps data confirm that trigger by ZDP,s sending

 *
 * @param       pMsg - confirm message
 *
* @return       NONE
 */
void ZDP_ProcessDataConfirm( afDataConfirm_t* pMsg )
{
  uint8_t status = pMsg->hdr.status;
  pfnAfCnfCB afCnfCB = pMsg->afCnfCB;
  void* cnfParam = pMsg->cnfParam;
  if( afCnfCB )
  {
    afCnfCB( status, pMsg->endpoint, pMsg->transID, pMsg->clusterID, cnfParam );
  }
}
                           ...
                           uint16_t nwkAddr, NodeDescriptorFormat_t *pNodeDesc )
{
  ...
  *pBuf = pNodeDesc->DescriptorCapability;
  ...
  ZDP_TxOptions = AF_MSG_ACK_REQUEST; //Enable APS ACK for Node_Desc_rsp,added by Luoyiming
  stat = fillAndSend( &(inMsg->TransSeq), &(inMsg->srcAddr), Node_Desc_rsp, len );
  ZDP_TxOptions = AF_TX_OPTIONS_NONE;
 ...
 *
 * @param       TransSeq - Transaction sequence number of request.
 ...
 * @return      ZStatus_t
 */
ZStatus_t ZDP_ServerDiscRsp( byte TransSeq, zAddrType_t *dstAddr, byte status,
                           uint16_t aoi, uint16_t serverMask, byte SecurityEnable )
{
  ...
  *pBuf++ = HI_UINT16( serverMask );
  ...
  ZDP_TxOptions = AF_MSG_ACK_REQUEST;
  stat = fillAndSend( &TransSeq, dstAddr, Server_Discovery_rsp, len );
  ZDP_TxOptions = AF_TX_OPTIONS_NONE;
                                  ...
                                  byte TcSignificance, byte SecurityEnable )
{
          ...
          || (dstAddr->addr.shortAddr == NWK_BROADCAST_SHORTADDR_DEVRXON)) )
  {
    ...
    //direct set permit_join,fixed by luoyiming 2019-07-19
    if(duration == 0xFF)
    {
      duration = 0xFE;
    }
    NLME_PermitJoiningRequest( duration );
    if( APSME_IsDistributedSecurity() || (ZG_BUILD_COORDINATOR_TYPE && ZG_DEVICE_COORDINATOR_TYPE) )
    {
